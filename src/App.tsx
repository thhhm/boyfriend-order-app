    ...
    <main>
      ...
      <section className="container service-section" id="services">
        ...
        <div className="toolbar">
          ...
          <label className="search-box">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="找找看..." aria-label="搜索服务" />
          </label>
        </div>
      ...
      {isCartOpen && <div className="overlay" onClick={() => setIsCartOpen(false)}>
        <aside className="cart-panel" onClick={(event) => event.stopPropagation()} aria-label="我的订单">
          <div className="panel-header"><div><p className="section-kicker">your selected services</p><h2>已选服务 <span>♡</span></h2></div><button className="close-button" onClick={() => setIsCartOpen(false)} aria-label="关闭已选服务">×</button></div>
          {cartDetails.length === 0 ? <div className="empty-cart"><div>🧺</div><h3>还没有选服务</h3><p>先看看菜单，选好后再告诉游杰。<br />不着急，慢慢挑。</p><button className="primary-button" onClick={() => setIsCartOpen(false)}>返回菜单</button></div> : <>
            ...
            <div className="order-form"><label htmlFor="order-time">希望什么时候安排？<span>*</span></label><input id="order-time" value={time} onChange={(event) => { setTime(event.target.value); setFormError('') }} placeholder="例如：今晚 7 点 / 周六下午" /><label htmlFor="order-note">想补充几句话吗？</label><textarea id="order-note" value={note} onChange={(event) => setNote(event.target.value)} placeholder="地点、偏好，或者想告诉游杰的话..." rows={3} />{formError && <p className="form-error" role="alert">{formError}</p>}<button className="primary-button submit-button" onClick={submitOrder} disabled={isSubmitting} aria-busy={isSubmitting}>{isSubmitting ? '正在发送…' : '发送服务请求'} {!isSubmitting && <span>♡</span>}</button></div>
          </>}
        </aside>
      </div>}

      {isSubmitted && <div className="success-toast" role="status"><div className="success-icon">💌</div><div><strong>服务请求已发送</strong><p>游杰会在「{time}」查看并回复你。</p></div><button onClick={() => setIsSubmitted(false)} aria-label="关闭提示">×</button></div>}
    </main>
  )
}
